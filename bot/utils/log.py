...
class TelegramHandler(logging.Handler):
    ...
    def __init__(self, bot: Bot, log_chat_id: int, max_message_length: int = 4096, timeout: float = 60) -> None:
        super().__init__()
    ...
    def split_logs(self) -> list[list[str]]:
        tmp_list: list[list[str]] = [[]]
        tmp_length = 0

        for i in self.buffer:
            if tmp_length + len(i) > self.max_message_length:
                tmp_list.append([])
                tmp_length = 0
...
def _get_telegram_handler() -> TelegramHandler:
    if settings.log_chat is None:
        msg = "LOG_CHAT must be configured to enable TelegramHandler."
...
def _get_console_handler() -> StreamHandler[Any]:
