...
class GetRepo(BaseMiddleware):
    async def __call__(
        ...
        us = data.get("event_from_user")

        if us and us.first_name in IGNORED_NAMES:
            return None
...
class GetChat(BaseMiddleware):
    async def __call__(
        ...
        chat = await repo.chats.get(chat_.id, *chat_options)

        if not chat:
            chat = await repo.chats.create_from_aiogram_model(chat_)
...
def setup_get_repo_middleware(dp: Dispatcher) -> None:
    """
...
def setup_get_user_middleware(dp: Dispatcher) -> None:
    """
...
def setup_get_chat_middleware(dp: Dispatcher) -> None:
    """
