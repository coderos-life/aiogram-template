...
class BaseRepo(Generic[Model], ABC):
    model: type[Model]
    ...
    @overload
    async def create_from_model(self, db_obj: Model, /) -> Model: ...

    @overload
    async def create_from_model(self, *db_obj: Model) -> tuple[Model, ...]: ...
    ...
    async def get(self, db_object_id: int, *options: Any) -> Model | None:
    ...
    @overload
    async def get_all(self, count: Literal[True]) -> int: ...

    @overload
    async def get_all(self, count: Literal[False] = False) -> Sequence[Model]: ...
    ...
    async def get_all(self, count: bool = False) -> int | Sequence[Model]:
        ...
        return cast(Sequence[Model], result.scalars().all())
