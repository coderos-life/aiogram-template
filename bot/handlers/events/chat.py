...
async def bot_added_in_chat(event: ChatMemberUpdated, repo: Repositories, chat: Chat) -> None:
    ...
    logger.info(
        f"Bot was added in chat {chat_.title}({chat_.id}",
    )
...
async def chat_boost(chat_boost: ChatBoostUpdated) -> None: ...


@router.removed_chat_boost(IS_GROUP)
async def remove_chat_boost(chat_boost: ChatBoostUpdated) -> None: ...
